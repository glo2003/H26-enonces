...
Github Actions, qui est intégré à Github.

Votre pipeline doit avoir au moins les étapes **séparées** et **explicites** suivantes :
...
3. Exécution du pipeline sur :
    - tous les commits de *pull requests*
    - tous les commits de chaque branche principale (ex : `dev`, `master`, etc.)

Il vous est fortement suggéré de lire la
...
2× moins sur `windows` et 10× moins sur `macos`) par équipe pour le pipeline CI.

Afin d'éviter de dépasser les minutes permises, vous pouvez configurer une limite de
temps par exécution avec `timeout-minutes`. Normalement, une exécution ne devrait
jamais dépasser **5 minutes** (dans votre cas).
